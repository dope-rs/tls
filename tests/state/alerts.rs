use crate::common::{established_pair, raw_client, signing_key};
use dope_tls::{Error, state::status::PeerClose};
...
fn fatal_alert_surfaces_description_and_closes() {
    let (mut client, mut server) = established_pair();
    server
        .send_fatal_alert(Description::HandshakeFailure)
        .unwrap();
    let wire = server.pull_send();
    let err = client.read_tcp(&wire).unwrap_err();
    assert_eq!(err, Error::PeerAlert(Description::HandshakeFailure));
    assert_eq!(
        client.peer_close(),
        PeerClose::Fatal(Description::HandshakeFailure)
    );
...
fn plaintext_close_notify_is_rejected_not_clean() {
    ...
    assert_eq!(
        ...
        Error::PeerAlert(Description::CloseNotify)
    );
    assert_eq!(
        client.peer_close(),
        PeerClose::Fatal(Description::CloseNotify)
    );
    assert!(client.is_closed());
}
...
fn alert_body_matches_shin_encoding() {
    assert_eq!(Alert::fatal(Description::HandshakeFailure).body(), [2, 40]);
