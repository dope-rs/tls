use crate::common::{ClientState, TestServer, pump, server_state};
use dope_tls::{Clock, Roots};
use rcgen::{CertificateParams, ExtendedKeyUsagePurpose, IsCa, KeyPair, PKCS_ED25519};
use shin::client::config::{OwnedTrustAnchor, Verifier};
use shin::crypto::sig::SigningKey;
use shin::identity::asn1::{Reader, Tag};
use shin::identity::cert::{Cert, SubjectPublicKeyInfo};
use shin::server::config::CertSource;

const HOSTNAME: &str = "host.local";

fn extract_ed25519_seed(pkcs8: &[u8]) -> Option<[u8; 32]> {
    let mut r = Reader::new(pkcs8);
    let inner = r.read_tagged(Tag::SEQUENCE).ok()?;
    let mut ir = Reader::new(inner);
    let _version = ir.read_tagged(Tag::INTEGER).ok()?;
    let _alg = ir.read_tagged(Tag::SEQUENCE).ok()?;
    let outer_oct = ir.read_tagged(Tag::OCTET_STRING).ok()?;
    let mut or = Reader::new(outer_oct);
    let inner_oct = or.read_tagged(Tag::OCTET_STRING).ok()?;
    if inner_oct.len() != 32 {
        return None;
    }
    let mut seed = [0u8; 32];
    seed.copy_from_slice(inner_oct);
    Some(seed)
}

fn ed25519_self_signed() -> (Vec<u8>, SigningKey) {
    let key = KeyPair::generate_for(&PKCS_ED25519).unwrap();
    let pkcs8 = key.serialize_der();
    let seed = extract_ed25519_seed(&pkcs8).expect("seed");
    let signing = SigningKey::from_seed(&seed).unwrap();
    let mut params = CertificateParams::new(vec![HOSTNAME.into()]).unwrap();
    params
        .distinguished_name
        .push(rcgen::DnType::CommonName, HOSTNAME);
    params.is_ca = IsCa::NoCa;
    params.extended_key_usages = vec![ExtendedKeyUsagePurpose::ServerAuth];
    let cert = params.self_signed(&key).unwrap();
    (cert.der().to_vec(), signing)
}

#[test]
fn webpki_roots_returns_nonempty_anchor_pool() {
    let pool: Vec<_> = Roots::new().collect();
    assert!(
        pool.len() > 50,
        "expected >= 50 Mozilla roots, got {}",
        pool.len()
    );
    for ta in &pool {
        assert!(!ta.subject_der.is_empty());
        assert!(!ta.spki_der.is_empty());
    }
}

#[test]
fn webpki_roots_spki_round_trips_through_verifier_parse() {
    let pool: Vec<_> = Roots::new().collect();
    assert!(!pool.is_empty());
    for ta in &pool {
        let spki = SubjectPublicKeyInfo::parse_standalone(&ta.spki_der).unwrap_or_else(|e| {
            panic!("anchor SPKI failed to parse: {e:?}");
        });
        assert!(
            !matches!(
                spki.algorithm,
                shin::identity::cert::algorithm::PublicKey::Unsupported
            ),
            "anchor SPKI uses an unsupported algorithm"
        );
        assert!(
            !spki.subject_public_key.is_empty(),
            "anchor SPKI missing public key bytes"
        );
    }
}

#[test]
fn from_cert_der_extracts_anchor_fields() {
    let (cert_der, _) = ed25519_self_signed();
    let cert = Cert::parse(&cert_der).unwrap();
    let anchor = OwnedTrustAnchor::from_cert_der(&cert_der).unwrap();
    assert_eq!(anchor.subject_der, cert.tbs.names.subject.as_der());
    assert_eq!(anchor.spki_der, cert.tbs.spki.raw_der);
}

#[test]
fn x509_handshake_round_trip() {
    let (cert_der, signing) = ed25519_self_signed();
    let cert = Cert::parse(&cert_der).unwrap();
    let nb = cert.tbs.validity.not_before;
    let na = cert.tbs.validity.not_after;
    let now = shin::identity::UnixTime((nb.0 + na.0) / 2)
        .as_secs()
        .expect("test certificate is post-epoch")
        .checked_mul(1000)
        .expect("test certificate time fits milliseconds");
    let anchor = OwnedTrustAnchor::from_cert_der(&cert_der).unwrap();

    let server_config = shin::server::config::Config {
        source: CertSource::X509 {
            chain_der: vec![cert_der.clone()],
            signing_key: signing,
        },
        alpn_protocols: Vec::new(),
        ticket_keys: None,
    };
    let shard = shin::server::Shard::new(server_config).expect("valid server shard");
    let mut server = TestServer::new(server_state(shard).expect("valid server buffer layout"));
    let mut client = ClientState::with_clock(
        shin::client::config::Config {
            verifier: Verifier::X509 {
                anchors: vec![anchor],
                hostname: HOSTNAME.as_bytes().to_vec(),
                certificate_limit: shin::client::config::CertificateLimit::ONE_RECORD,
            },
            transport_params: Vec::new(),
            alpn_protocols: Vec::new(),
            enable_early_data: false,
        },
        Clock::FixedMillis(now),
    )
    .unwrap();

    pump(&mut client, &mut server);
    assert!(client.is_established(), "client x509 handshake established");
    assert!(server.is_established(), "server x509 handshake established");

    let req = b"x509 ping";
    client.write_app(req).unwrap();
    pump(&mut client, &mut server);
    let got = server.pull_app().expect("server got app data");
    assert_eq!(got.as_slice(), req);
}
