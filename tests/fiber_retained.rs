...
type Pool<'scope, 'd> = Listener<'scope, 'd, 0, Tcp, Tls>;
...
#[test]
fn fragmented_and_provided_plaintext_share_one_retained_cursor() {
    ...
    let executor = Executor::new(dope::driver::Config::for_tcp_profile::<Balanced>(
        MAX_CONNECTIONS,
    ))
    .expect("executor")
    .with_storage_factory(ListenerPort::<Tls>::factory(MAX_CONNECTIONS).expect("listener storage"));

    executor.enter(|mut session| {
        let hash = session.seed().derive(dope::hash::domain::ACCEPT).state();
        let (storage, mut driver) = session.storage_and_driver();
        ...
        let received = session
            .with_app(App { pool }, |mut app| {
                ...
                app.block_on(dope_gen::fiber!('_ => async move {
                    ...
                    while received.len() < expected {
                        ...
                        let (result, bytes) = stream.read(Vec::with_capacity(want)).await;
                        result?;
                        if bytes.is_empty() {
